...
const conversationListToMap = (
  ...
  return conversationList.reduce<ConversationsMap>((obj, item) => {
    obj[item.personUuid] = item;
    return obj;
  }, {});
};
...
const populateConversation = async (
  ...
  const apiData = (
    await japi("post", "/inbox-info", {
      person_uuids: [conversation.personUuid],
    })
  ).json;
  await populateConversationList([conversation], apiData);
};

const jidToBareJid = (jid: string): string => jid.split("@")[0];

const personUuidToJid = (personUuid: string): string =>
  `${personUuid}@bunk-app.in`;

const setInboxSent = (recipientPersonUuid: string, message: string) => {
  const i = _.cloneDeep(getInbox() ?? emptyInbox());

  const chatsConversation = i.chats.conversationsMap[recipientPersonUuid] as
    | Conversation
    | undefined;
  const introsConversation = i.intros.conversationsMap[recipientPersonUuid] as
    | Conversation
    | undefined;
...
const setInboxRecieved = async (fromPersonUuid: string, message: string) => {
  ...
  const chatsConversation = inbox.chats.conversationsMap[fromPersonUuid] as
    | Conversation
    | undefined;
  const introsConversation = inbox.intros.conversationsMap[fromPersonUuid] as
    | Conversation
    | undefined;
  const archiveConversation = inbox.archive.conversationsMap[fromPersonUuid] as
    | Conversation
    | undefined;
...
const authenticate = async () => {
  ...
  await Promise.all([refreshInbox(), getAndRegisterPushToken()]);
};
...
> => {
  const { numTries = 3, timeoutMs = messageTimeout } = config ?? {};
  ...
  if (
    ...
    return sendMessage(recipientPersonUuid, content, id, {
      numTries: numTries - 1,
      timeoutMs,
    });
  } else {
...
const conversationsToInbox = (conversations: Conversation[]): Inbox => {
  const chats = conversations.filter((c) => c.location === "chats");
  const intros = conversations.filter((c) => c.location === "intros");
  const archive = conversations.filter((c) => c.location === "archive");
...
const setConversationArchived = (personUuid: string, isSkipped: boolean) => {
  ...
  const conversationToUpdate = (inbox.chats.conversationsMap[personUuid] ??
    inbox.intros.conversationsMap[personUuid] ??
    inbox.archive.conversationsMap[personUuid]) as Conversation | undefined;
...
const fetchConversation = async (
  ...
  const sentinelDetector = (doc: any) => {
    ...
    const expectedDoc = {
      iq: {
        "@xmlns": "jabber:client",
        "@from": `${credentials.username}@bunk-app.in`,
        "@to": `${credentials.username}@bunk-app.in`,
        "@id": queryId,
        "@type": "result",
        fin: {
          "@xmlns": "urn:xmpp:mam:2",
      ...
      const {
        message: {
          result: {
            ...
            forwarded: {
              delay: { "@stamp": timestamp },
              message: { "@from": from, "@to": to, body: text },
            },
          },
        },
      } = doc;
