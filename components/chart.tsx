...
const Chart = ({ name1, percentage1, name2, percentage2, ...props }) => {
  const { children, dimensionName, minLabel, maxLabel } = props;
  ...
  if (dimensionName && (minLabel || maxLabel)) {
    throw new Error(
      "dimensionName can't be set at the same time as minLabel or maxLabel",
    );
  }
  ...
  const labelPercentage = (percentage: number | undefined) => {
    ...
    if (minLabel || maxLabel)
      return clamp(2 * Math.abs(percentage - 50), 0, 100);
    return percentage;
  };
    ...
    ({ position, color, ...props }) => {
      ...
      return (
        <>
          ...
          {position_ !== undefined && label === null && (
            <DefaultText
              style={{
                position: "absolute",
                ...labelPosition,
          ...
          {position_ === undefined && (label || label === null) && (
            <View
              style={{
                width: "100%",
                position: "absolute",
      ...
      >
        ...
        >
          ...
          >
            ...
            {(minLabel || maxLabel) && (
              <DefaultText
                style={{ flex: 3, textAlign: "center", color: "#666" }}
              >
                0%
              </DefaultText>
            )}
            {!minLabel && !maxLabel && (
              <DefaultText
                style={{ flex: 3, textAlign: "center", fontWeight: "500" }}
              >
                {dimensionName}
              </DefaultText>
            )}
          ...
          {minLabel && maxLabel && (
            ...
            >
              ...
              <DefaultText
                style={{ fontWeight: "500", flex: 10, textAlign: "right" }}
              >
                {maxLabel}
              </DefaultText>
            </View>
          ...
          <>
            ...
            >
              ...
              {!(minLabel && maxLabel) && percentage1 !== undefined && (
                <>
                  {"\n\n"}A score of {percentage1}% means that about{" "}
                  {100 - percentage1}% people on Bunk scored higher than that,
                  and about {percentage1}% scored lower.
                </>
              )}
              {minLabel && maxLabel && (
                <>
        ...
        {(!(minLabel && maxLabel) || expanded) && <TraitInfoButton />}
      </Pressable>
    </Animated.View>
  );
};
